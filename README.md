...
## ⚠️ Note on 261-District Recomputation (2026-05-19)

The dataset and analytical pipeline have been **migrated to 261 districts** (from 260), with **5 duplicate records removed** and **Guan District (Oti Region) formally added**. 

**Key changes:**
- ✅ Master CSV deduplicated: 265 rows → 260 unique + 1 Guan = **261 total**
- ✅ All spatial weights recomputed (KNN-8 on district centroids)
- ✅ Canonical Moran's I, LISA, GWR values recomputed (new 261-district statistics)
- ✅ ML pipeline retrained (RF, XGB, LightGBM, SHAP on 261 districts)
- ✅ Test suite updated with 261-district canonical values (seed=42, reproducible)
...
**Script for reproducibility:** `scripts/deduplicate_master_dataset.py`
...
| Metric | Value | Notes |
|--------|-------|-------|
| Global Moran's I (HIV-TB co-infection) | **0.810** | p < 0.001, 999 perms |
...
| **Districts analysed** | **261** | Post-2018 health districts; deduplicated 2026-05 |
...
- Estimated runtime: ~15–20 minutes on a standard laptop (LightGBM ~5 min)
...
- **Master dataset (original, archival):** `outputs/data/Ghana_HIV_TB_Master_Dataset.csv`
