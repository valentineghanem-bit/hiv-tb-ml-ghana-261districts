...
| **Repository name** | `hiv-tb-ml-ghana-260districts` | `hiv-tb-ml-ghana-261districts` | (pending GitHub rename) |
...
**Rationale:** Rows with missing Classification (marked empty string) were deprioritized for removal when duplicates existed. Rows with complete Classification fields were kept.
...
## Phase 2: Canonical Statistics Recomputation (Completed 2026-05-19)
...
**Current:** `hiv-tb-ml-ghana-260districts`  
**Target:** `hiv-tb-ml-ghana-261districts`

**GitHub steps (admin only):**
1. Settings → General → Repository name
2. Update from `260districts` to `261districts`
3. GitHub auto-redirects old URLs (30 days grace period)
4. Update branch protection rules (if any)
...
- [x] **Branch created & reviewed:** `migrate-to-261-districts` ready for PR

---

## Phase 5: Post-Merge Workflow

### Immediate (Upon PR Merge)

1. **Rename repository** on GitHub (Settings → General)
2. **Update GitHub Pages** (if using; redirect URLs)
3. **Tag v1.1.0** on main branch:
   ```bash
   git tag -a v1.1.0 -m "261-district migration: deduplication, Guan addition, canonical stats recomputed"
   git push origin v1.1.0
   ```
4. **Run CI/CD pipeline** to validate all tests pass on main
...
2. **Publish migration summary** (optional):
   - Tweet/LinkedIn post: "Updated all HIV/TB Ghana repos to 261 districts (post-2018 health reform). Duplicates removed, Guan added, full pipeline recomputed with seed=42. See MIGRATION_GUIDE.md"
